...
if __name__ == '__main__':
  ...
  for line in fread.readlines()[1:]:
    ...
    print(video_feat_filepath)
    print(sound_feat_filepath)
    ...
    if os.path.exists(video_feat_filepath) and os.path.exists(sound_feat_filepath):
      ...
      print(compressed_sound.shape)

      video = np.genfromtxt(video_feat_filepath, delimiter=";", dtype="float")

      complete_feat = np.concatenate((video, compressed_sound))
      print(complete_feat.shape)

      feat_list.append(complete_feat)
