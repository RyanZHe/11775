...
def extract_feat(model, sound_input, input_name, config):
    ...
    for idx in xrange(layer_min, layer_max):
        ...
        if config.is_save:
            np.save(os.path.join(config.outpath, 'tf_fea{}{}.npy'.format( \
                input_name[-14:-4], str(idx).zfill(2))), np.squeeze(feature))
            print("Save layer {} with shape {} as {}/tf_fea{}{}.npy".format( \
                    idx, np.squeeze(feature).shape, config.outpath, input_name[-14:-4], str(idx).zfill(2)))
...
if __name__ == '__main__':
    ...
    with tf.Session(config=sess_config) as session:
        ...
        for idx in xrange(len(sound_samples)):
            output = extract_feat(model, sound_samples[idx], txt_list[idx], args)
